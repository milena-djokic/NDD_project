...
# Author: Chop Yan Lee, Milena Djokic
...
class Prediction_folder:
    ...
    def __init__(self,prediction_folder,save_dest,num_model=5,project_name=None):
    ...
    def parse_ranking_debug_file(self):
        ...
        if not os.path.exists(os.path.join(self.prediction_folder,'ranking_debug.json')):
            ...
            with open(os.path.join(self.prediction_folder,'ranking_debug.json'), 'r') as f:
                data = json.load(f)
            self.rank_to_model = {f'ranked_{i}':'_'.join(model.split('_')[:2]) for i, model in enumerate(data.get("order"))}
    ...
    def parse_prediction_fasta_file(self):
        ...
        """
        fasta_path = self.prediction_folder
        fasta_path = fasta_path.replace('/output/', '/input/')
        fasta_path = re.sub(r'/run(\d+)/', r'/\1/', fasta_path)
        fasta_path = str('/'.join(fasta_path.split('/')[:-2])) +  '/' + fasta_path.split('/')[-1] + '.fasta'
    ...
    def assign_model_info(self):
        ...
        for model_id, model_inst in self.model_instances.items():
            ...
            model_inst.save_dest = self.save_dest
            model_inst.prediction_name = self.prediction_name
    ...
    def write_out_calculated_metrics(self,project_name=None):
        ...
        """
        metrics_out_path = os.path.join(self.save_dest, f'template_indep_info_{self.prediction_name}.tsv')
        ...
        if not self.predicted:
            row = common_info + ['Prediction failed']
            # the length of row is 5 at this point, so we don't wanna hardcode this
            none_vals = [None]*(len(metrics_columns_dtype)-len(row))
            row = row + none_vals
            metrics_df.loc[len(metrics_df)] = row
        else:
            # insert metric info in a row-wise manner
            for model_id, model_inst in self.model_instances.items():
                model_name=self.rank_to_model.get(model_id)
                row = common_info + [model_name] + [model_id]
                for column in list(metrics_columns_dtype)[6:]:
    ...
    def write_out_contacts(self):
        ...
        Returns:
            {self.run_id}_all_model_contacts_{self.prediction_name}.tsv: A tsv file with the calculated information of atom-atom contact in all predicted models
        """
        contact_out_path = os.path.join(self.save_dest, f'all_model_contacts_{self.prediction_name}.tsv')
        ...
        if self.predicted:
            ...
            pass
        if not os.path.exists(contact_out_path):
...
class Predicted_model:
    ...
    def __init__(self,predicted_model):
        ...
        self.path_to_prediction_folder = None
    ...
    def calculate_structural_metric(self):
        ...
        """
        cmd.create(self.prediction_name, 'all')
        ...
        cmd.color('magenta','all and chain B')
        ...
        if not os.path.exists(f'{os.path.join(self.save_dest,self.predicted_model)}_contacts_{self.prediction_name}.pse'):
            cmd.save(f'{os.path.join(self.save_dest,self.predicted_model)}_contacts_{self.prediction_name}.pse')
        # pymol was really difficult to work with and it's very sensitive to any changes, keep that in mind
    ...
    def get_model_independent_metrics(self):
        ...
        print(f'{self.prediction_name}, {self.predicted_model}')
    ...
    if (path_to_run is None) and (path_to_prediction is None):
        ...
        for run_id in run_ids.split(','):
            ...
            for directory in os.listdir(run_path):
                for file in os.listdir(os.path.join(run_path,directory)):
                    if file != 'log_dir':
                        file_abs = os.path.join(run_path,directory,file)
                        save_dest='/'.join(file_abs.split('/')[:-1]) + '/model_eval/'
                        if not os.path.exists(save_dest):
                            os.makedirs(save_dest) # if the folder doesn't exist, we create it
                            folder = Prediction_folder(file_abs,save_dest,num_model=5,project_name=project_name)
                            folder.process_all_models()
                            folder.write_out_calculated_metrics()
                            if skip_contacts:
                                continue
                            ...
                            continue # otherwise it means this has already been processed
                    else:
                        continue
